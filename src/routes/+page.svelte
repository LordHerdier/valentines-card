<script lang="ts">
  ...
  let heartBoostUntil = 0;

  function boostHearts(ms = 2600) {
    heartBoostUntil = performance.now() + ms;
  }

   type NoteLine = { html: string; kind?: "greeting" | "signature" | "" };

  const noteLines: NoteLine[] = [
    { html: "My dearest Fayrene,", kind: "greeting" },
    { html: "These months spent with you have felt" },
    { html: "like a blissful, unwaking dream, where" },
    { html: "the wind is filled with the scent of your" },
    { html: "perfume and every moment is painted in" },
    { html: "the softest hues of happiness." },
    { html: "<br/>" },
    { html: "Thank you for being you." },
    { html: "For your kindness, your warmth," },
    { html: "and the way you make me feel" },
    { html: "so safe when you're around." },
    { html: "Forever yours,<br/>Charlotte ♥", kind: "signature" },
  ];

  // Timing knobs (tweak these, not 10 inline styles)
  const NOTE_START_DELAY_MS = 650; // pause before first line starts typing
  const NOTE_GAP_MS = 48;         // pause between lines
  const NOTE_MS_PER_CHAR = 32;     // longer lines take longer
  const NOTE_MIN_DUR_MS = 200;
  const NOTE_MAX_DUR_MS = 2200;

  function noteLineDurMs(i: number) {
    const raw = noteLines[i]?.html ?? "";
    const text = raw.replace(/<br\s*\/?>/gi, " ");
    const est = text.length * NOTE_MS_PER_CHAR;
    return Math.max(NOTE_MIN_DUR_MS, Math.min(NOTE_MAX_DUR_MS, est));
  }

  function noteLineDelayMs(i: number) {
    let acc = NOTE_START_DELAY_MS;
    for (let k = 0; k < i; k++) acc += noteLineDurMs(k) + NOTE_GAP_MS;
    return acc;
  }
  ...
  function tick(t: number) {
    ...
    for (let i = 0; i < hearts.length; i++) {
      const he = hearts[i];
      const boosting = performance.now() < heartBoostUntil;
      const speedMult = boosting ? 2.0 : 1;
      const alphaMult = boosting ? 1.15 : 1;

      he.y += he.speed * speedMult * dt;
      ...
      ctx.fillStyle = `rgba(${he.color.r},${he.color.g},${he.color.b},${he.alpha * alphaMult})`;
    ...
    <div class="note">
      <div class="note-content">
        {#each noteLines as line, i}
          <p
            class={`handwritten typed-line ${line.kind ?? ""}`}
            style={`--delay:${noteLineDelayMs(i)}ms; --dur:${noteLineDurMs(i)}ms;`}
          >
            {@html line.html}
          </p>
        {/each}
      ...
      <div class="polaroids">
        <div class="polaroid polaroid-1">
          <div class="polaroid-photo">
            <img class="photo-img" src="/photos/pool_party.JPG" alt="Pool party" />
          </div>
          <p class="polaroid-caption">the early parties ♡</p>
          <div class="polaroid-note">
            <p class="note-text">
              the earliy parties where we first met...
                i was drawn to your kindness and energy
            </p>
          </div>
        </div>
        ...
        <div class="polaroid polaroid-2">
          <div class="polaroid-photo">
            <div class="polaroid-photo">
            <img class="photo-img" src="/photos/movie_marathon.JPG" alt="Movie marathon" />
          </div>
          </div>
          <p class="polaroid-caption">movie marathon ♡</p>
          <div class="polaroid-note">
            <p class="note-text">
              hours flew by like minutes.
              i knew i wanted to spend them all with you.
            </p>
          </div>
        </div>
        ...
        <div class="polaroid polaroid-3">
          <div class="polaroid-photo">
            <div class="polaroid-photo">
              <img class="photo-img" src="/photos/first_date_1.JPG" alt="First date" />
            </div>
          </div>
          <p class="polaroid-caption">our first date ♡</p>
          <div class="polaroid-note">
            <p class="note-text">
              the gardens were beautiful, but you were the most beautiful thing there.
            </p>
          </div>
        </div>
        <div class="polaroid polaroid-4">
          <div class="polaroid-photo">
            <div class="polaroid-photo">
              <img class="photo-img" src="/photos/lazy_mornings.JPG" alt="Lazy mornings" />
            </div>
          </div>
          <p class="polaroid-caption">all the mornings we've spent ♡</p>
          <div class="polaroid-note">
            <p class="note-text">
        ...
        <div class="polaroid polaroid-7">
          <div class="polaroid-photo">
            <div class="polaroid-photo">
              <img class="photo-img" src="/photos/holidays_2.JPEG" alt="cuddles" />
            </div>
          </div>
          <p class="polaroid-caption">and of course the cuddles ♡</p>
          <div class="polaroid-note">
            <p class="note-text">
        ...
        <div class="polaroid polaroid-5">
          <div class="polaroid-photo">
            <div class="polaroid-photo">
              <img class="photo-img" src="/photos/mexican_food.JPG" alt="Mexican food" />
            </div>
          </div>
          <p class="polaroid-caption">to all the dinner dates ♡</p>
          <div class="polaroid-note">
            <p class="note-text">
                from mexican to sushi, every meal is better with you
            </p>
          </div>
        </div>
        <div class="polaroid polaroid-6">
          <div class="polaroid-photo">
            <div class="polaroid-photo">
              <img class="photo-img" src="/photos/holidays_1.JPEG" alt="Holidays" />
            </div>
          </div>
          <p class="polaroid-caption">and the holidays in between ♡</p>
          <div class="polaroid-note">
            <p class="note-text">
                every holiday is my favorite with you, but valentines is extra special
            </p>
          </div>
        </div>
        <div class="polaroid polaroid-8">
          <div class="polaroid-photo">
            <div class="polaroid-photo">
              <img class="photo-img" src="/photos/game_nights.JPG" alt="game nights" />
            </div>
          </div>
          <p class="polaroid-caption">game nights ♡</p>
          <div class="polaroid-note">
            <p class="note-text">
                and all the fun nights in between
            </p>
          </div>
        </div>
        <div class="polaroid polaroid-9">
          <div class="polaroid-photo">
            <div class="polaroid-photo">
              <img class="photo-img" src="/photos/get_cherished.JPEG" alt="Get Cherished Nerd" />
            </div>
          </div>
          <p class="polaroid-caption">♡</p>
          <div class="polaroid-note">
            <p class="note-text">
                I can't wait to spend all the future moments with you, big and small. Happy Valentine's Day, my love.
            </p>
          </div>
        </div>
      </div>
...
<style>
  ...
  .typed-line {
    ...
    animation: typeReveal var(--dur, 1200ms) linear forwards;
  ...
  /* preserve your “first line is bigger” vibe, but via class (not :first-child) */
  ...
  /* signature should still be signature-y */
  ...
  .note-content .handwritten {
      margin-bottom: 4px; /* try 8–12px */
    }
  ...
  .polaroid:hover {
    transform: translateY(-6px) rotate(var(--rotate, 0deg)) scale(1.03);
    box-shadow:
